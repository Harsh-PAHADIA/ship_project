...
import { RouteRow, CBRecord } from '../../core/domain/types';
...
let inMemoryCB: CBRecord[] = [];
let inMemoryBankEntries: any[] = [];
...
export const repo: Repository = {
  async getAllRoutes(): Promise<RouteRow[]> {
    if (!usePostgres) {
  ...
  async getBaseline(): Promise<RouteRow | null> {
    if (!usePostgres) {
