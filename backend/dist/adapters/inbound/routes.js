...
const repo_1 = require("../outbound/repo");
...
router.get('/routes', async (req, res) => {
    const routes = await repo_1.repo.getAllRoutes();
    res.json(routes);
});
// POST /api/routes/:routeId/baseline
router.post('/routes/:routeId/baseline', async (req, res) => {
    const { routeId } = req.params;
    await repo_1.repo.setBaseline(routeId);
    res.status(204).send();
});
// GET /api/routes/comparison
router.get('/routes/comparison', async (req, res) => {
    const baseline = await repo_1.repo.getBaseline();
    if (!baseline)
        return res.status(404).json({ message: 'No baseline set' });
    const others = await repo_1.repo.getOtherRoutes();
    const comparisons = others.map((o) => {
        const percentDiff = ((o.ghg_intensity / baseline.ghg_intensity) - 1) * 100;
        const compliant = o.ghg_intensity <= computeCB_1.TARGET_INTENSITY;
        return {
            routeId: o.route_id,
...
router.get('/compliance/cb', async (req, res) => {
    ...
    if (!shipId)
        return res.status(400).json({ message: 'shipId or routeId required' });
    const routes = await repo_1.repo.getAllRoutes();
    const r = routes.find((x) => x.route_id === shipId && x.year === year);
    if (!r)
        return res.status(404).json({ message: 'Route not found for shipId/year' });
    const cb = (0, computeCB_1.computeCB)(computeCB_1.TARGET_INTENSITY, r.ghg_intensity, r.fuel_consumption);
    await repo_1.repo.upsertCBRecord({ ship_id: shipId, year, cb_tonnes: cb });
...
router.get('/compliance/adjusted-cb', async (req, res) => {
    ...
    const year = Number(req.query.year || 2024);
    if (!shipId)
        return res.status(400).json({ message: 'shipId required' });
    const cbRec = await repo_1.repo.getCBRecord(shipId, year);
    const bank = await repo_1.repo.getBankedAmount(shipId, year);
...
router.post('/banking/bank', express_1.default.json(), async (req, res) => {
    ...
    if (!shipId || !year || typeof amount !== 'number')
        return res.status(400).json({ message: 'shipId, year, amount required' });
    if (amount <= 0)
        return res.status(400).json({ message: 'Amount must be > 0' });
    await repo_1.repo.addBankEntry(shipId, year, amount);
    res.status(201).json({ shipId, year, banked: await repo_1.repo.getBankedAmount(shipId, year) });
});
router.post('/banking/apply', express_1.default.json(), async (req, res) => {
    const { shipId, year, amount } = req.body;
    if (!shipId || !year || typeof amount !== 'number')
        return res.status(400).json({ message: 'shipId, year, amount required' });
    const available = await repo_1.repo.getBankedAmount(shipId, year);
    if (amount > available)
        return res.status(400).json({ message: 'Insufficient banked amount' });
    await repo_1.repo.applyBankEntry(shipId, year, amount);
    res.status(200).json({ shipId, year, applied: amount, banked: await repo_1.repo.getBankedAmount(shipId, year) });
});
// POST /api/pools
router.post('/pools', express_1.default.json(), async (req, res) => {
    const { year, members } = req.body;
    if (!Array.isArray(members) || typeof year !== 'number')
        return res.status(400).json({ message: 'year and members required' });
    try {
        const result = (0, createPool_1.createPool)(members);
        res.json({ year, members: result, sum: result.reduce((s, m) => s + (m.cbAfter || 0), 0) });
    }
    catch (err) {
